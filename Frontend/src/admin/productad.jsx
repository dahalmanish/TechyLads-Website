...
export default function Productad() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get("https://techy-lads-website.vercel.app/api/pds");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedProduct) {
        ...
        const response = await axios.put(`https://techy-lads-website.vercel.app/api/products/${selectedProduct._id}`, cardData);
        if (response.status === 200) {
          alert("Product updated successfully!");
        }
      } else {
        // If no product is selected, create a new one
        const response = await axios.post("https://techy-lads-website.vercel.app/api/products", cardData);
  ...
  const handleDelete = async (productId) => {
    try {
      const response = await axios.delete(`https://techy-lads-website.vercel.app/api/products/${productId}`);
      if (response.status === 200) {
